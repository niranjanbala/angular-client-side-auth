...
module.exports = {
    ...
    validate: function(user) {
        //check(user.username, 'Email must be 1-20 characters long').len(1, 20);
    ...
    localStrategy: new LocalStrategy(
        function(username, password, done) {            
            HttpPost.PostRequest('/v3/user/login',{
                'user_logon':username,
                ...
                if(resp.success==true) {
                    return done(null, {
                        id: resp.response.userId,
                        token: resp.response.server_auth_token,
                        username:   resp.response.userName,
                        role:   userRoles.user
                    });
                } else {
                    done(null, false, resp);    
                }                
            });
        }
    ),
